<template>
        ...
        <table class="table_header" border="1" cellspacing="0" cellpadding="0">
            <tr class="table_header__row">
                <td v-for="(item, index) in columnList" :key="item.key" @click="_sortBy(index, item.key, item.sort)"
                    ...
                    {{ item.title }}
                    <span v-if="item.sort"
                        class="table_header__item_up"
                        :class="`table_header__item_up--${item.sortType === 1 ? 'active' : ''}`"></span>
                    <span v-if="item.sort" 
                        class="table_header__item_down"
    ...
    <div class="paging">
        共{{ totalNum }}条记录
        每页{{ perPage }} 条
        <span @click="_prevPage" class="paging__btn" :class="`paging__btn--${isFirst ? '' : 'active'}`">上一页</span>
        <span @click="_nextPage" class="paging__btn" :class="`paging__btn--${isLast ? '' : 'active'}`">下一页</span>
        前往第<input type="text" class="paging__input" :class="`paging__input--${isValid ? '': 'error' }`" :value="curIndex" @input.sync="_setValue($event)"> 页
    </div>
</template>
